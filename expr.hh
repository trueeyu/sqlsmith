/// @file
/// @brief grammar: Value expression productions

#pragma once

#include "prod.hh"
#include <string>

using std::shared_ptr;
using std::vector;
using std::string;

struct value_expr : prod {
    sqltype *type{};

    void out(std::ostream &out) override = 0;

    virtual ~value_expr() = default;

    explicit value_expr(prod *p) : prod(p) {}

    static shared_ptr<value_expr> factory(prod *p, sqltype *type_constraint = nullptr);
};

// TODO: add later
struct case_expr : value_expr {
    shared_ptr<value_expr> condition;
    shared_ptr<value_expr> true_expr;
    shared_ptr<value_expr> false_expr;

    explicit case_expr(prod *p, sqltype *type_constraint = nullptr);

    void out(std::ostream &out) override;

    void accept(prod_visitor *v) override;
};

struct funcall : value_expr {
    routine *proc;
    bool is_aggregate;
    vector<shared_ptr<value_expr> > parms;

    void out(std::ostream &out) override;

    ~funcall() override = default;

    explicit funcall(prod *p, sqltype *type_constraint = nullptr, bool agg = false);

    void accept(prod_visitor *v) override {
        v->visit(this);
        for (const auto &p : parms)
            p->accept(v);
    }
};

struct const_expr : value_expr {
    std::string expr;

    explicit const_expr(prod *p, sqltype *type_constraint = nullptr);

    void out(std::ostream &out) override { out << expr; }

    ~const_expr() override = default;
};

struct column_reference : value_expr {
    explicit column_reference(prod *p, sqltype *type_constraint = nullptr);

    void out(std::ostream &out) override { out << reference; }

    std::string reference;

    ~column_reference() override = default;
};

struct coalesce : value_expr {
    const char *abbrev_;
    vector<shared_ptr<value_expr> > value_exprs;

    ~coalesce() override = default;;

    explicit coalesce(prod *p, sqltype *type_constraint = nullptr, const char *abbrev = "coalesce");

    void out(std::ostream &out) override;

    void accept(prod_visitor *v) override {
        v->visit(this);
        for (const auto &p : value_exprs)
            p->accept(v);
    }
};

struct nullif : coalesce {
    ~nullif() override = default;;

    explicit nullif(prod *p, sqltype *type_constraint = nullptr)
            : coalesce(p, type_constraint, "nullif") {};
};

struct bool_expr : value_expr {
    ~bool_expr() override = default;

    explicit bool_expr(prod *p) : value_expr(p) { type = scope->schema->booltype; }

    static shared_ptr<bool_expr> factory(prod *p);

    static shared_ptr<bool_expr> factory_for_or(prod *p);
};

struct truth_value : bool_expr {
    ~truth_value() override = default;

    const char *op;

    void out(std::ostream &out) override { out << op; }

    explicit truth_value(prod *p) : bool_expr(p) {
        op = ((d6() < 4) ? scope->schema->true_literal : scope->schema->false_literal);
    }
};

struct null_predicate : bool_expr {
    ~null_predicate() override = default;

    const char *negate;
    shared_ptr<value_expr> expr;

    explicit null_predicate(prod *p) : bool_expr(p) {
        negate = ((d6() < 4) ? "not " : "");
        expr = value_expr::factory(this);
    }

    void out(std::ostream &out) override {
        out << *expr << " is " << negate << "NULL";
    }

    void accept(prod_visitor *v) override {
        v->visit(this);
        expr->accept(v);
    }
};

struct exists_predicate : bool_expr {
    shared_ptr<struct query_spec> subquery;

    ~exists_predicate() override = default;

    explicit exists_predicate(prod *p);

    void out(std::ostream &out) override;

    void accept(prod_visitor *v) override;
};

struct bool_binop : bool_expr {
    shared_ptr<value_expr> lhs, rhs;

    explicit bool_binop(prod *p) : bool_expr(p) {}

    void out(std::ostream &out) override = 0;

    void accept(prod_visitor *v) override {
        v->visit(this);
        lhs->accept(v);
        rhs->accept(v);
    }
};

struct bool_term : bool_binop {
    ~bool_term() override = default;

    const char *op;

    void out(std::ostream &out) override {
        out << "(" << *lhs << ") ";
        indent(out);
        out << op << " (" << *rhs << ")";
    }

    explicit bool_term(prod *p) : bool_binop(p) {
        if (d6() < 4) {
            op = "or";
            lhs = bool_expr::factory_for_or(this);
            rhs = bool_expr::factory_for_or(this);
        } else {
            op = "and";
            lhs = bool_expr::factory(this);
            rhs = bool_expr::factory(this);
        }
    }
};

struct comparison_op : bool_binop {
    op *oper;

    explicit comparison_op(prod *p);

    ~comparison_op() override = default;;

    void out(std::ostream &o) override {
        o << *lhs << " " << oper->name << " " << *rhs;
    }
};

// TODO: add later
struct window_function : value_expr {
    void out(std::ostream &out) override;

    ~window_function() override = default;

    window_function(prod *p, sqltype *type_constraint);

    vector<shared_ptr<column_reference> > partition_by;
    vector<shared_ptr<column_reference> > order_by;
    shared_ptr<funcall> aggregate;

    static bool allowed(prod *pprod);

    void accept(prod_visitor *v) override {
        v->visit(this);
        aggregate->accept(v);
        for (const auto &p : partition_by)
            p->accept(v);
        for (const auto &p : order_by)
            p->accept(v);
    }
};
